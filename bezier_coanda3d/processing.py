...
def single_run(cmu_target, cmu_command, re_command, rho, mu, c, b, beta, eps, urf, parallel=False):
    import os
    import time
    ...
    mesh_command = "python3 autoMesh.py --clean true --mdot {} --meanFlow {} --airfoil NACA0015 --beta {}".format(mdot, U, beta)
    ...
    if abs(e) < eps:
        ...
        save_command = "python3 autoMesh.py --store True --runName /3d/beta{}/cmu{}/re{}".format(beta,cmu_target,re_command)
        ...
        time.sleep(5)
        single_run(cmu_target, cmu_command + kp*e, re_command, rho, mu, c, b, beta, eps, urf, parallel)
