...
parser.add_argument('--beta', default = 0.0, type = float, help = 'Sideslip angle')
...
def change_line_beta(U_filename,beta):
    line = 25
    with open(U_filename,'r') as f:
        test_lines = f.readlines()
        f.close()
    new_line = 'beta\t\t\t\t\t\t\t\t{};\t\t\t\t // side slip angle\n'.format(beta)
...
change_line_beta(U_filename,beta)
