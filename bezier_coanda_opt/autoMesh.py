...
if iteration == 0:
    ...
    cp_up, cp_lo = initialize_control_points(num_points, upper, lower, chord_length)
    # volume = volume_calculation(cp_up,cp_lo)
    # input("Test Volume: {}".format(volume))
    if fd[0] == 0:
        if path.isfile('{}/optimization/{}/0_cpu.txt'):
            pass
        else:
            os.system('mkdir -p {}/optimization/{}/'.format(airfoil_dir,runName))
        np.savetxt('{}/optimization/{}/0_cpu.txt'.format(airfoil_dir,runName),cp_up)
        np.savetxt('{}/optimization/{}/0_cpl.txt'.format(airfoil_dir,runName),cp_lo)
